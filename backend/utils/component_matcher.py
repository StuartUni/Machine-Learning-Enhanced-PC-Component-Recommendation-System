...
#     # If CPU upgraded, re-check Motherboard
#     if upgrades_made and matched.get("CPU"):
#         cpu_socket = matched["CPU"].get("socket", "").replace("FCLGA", "LGA")  # Normalize socket
...
#     matched = {"CPU": None, "Motherboard": None, "GPU": None, "RAM": None, "PSU": None, "Case": None}
#     total_cost = 0
...
#     # ✅ Smart GPU Priority if Budget ≥ 1000
#     if budget >= 1000:
...
#     else:
#         print("💻 Normal budget — matching CPU and GPU normally.")
...
#     # ✅ GPU Matching fallback
#     if matched["GPU"] is None and game_requirements.get("GPU", "Unknown") != "Unknown":
#         for gpu_candidate in game_requirements["GPU"].split(" or "):
#             match = match_best_component(gpu_candidate.strip(), gpu_data, "name")
#             if match and match["original_price"] <= gpu_budget:
#                 matched["GPU"] = match
#                 total_cost += match["original_price"]
#                 break
...
#     if matched["CPU"]:
#         cpu_socket = matched["CPU"]["socket"]
#         compatible_mobos = motherboard_data[motherboard_data["socket"] == cpu_socket]
#         if not compatible_mobos.empty:
#             best_mobo = compatible_mobos.sort_values("original_price").iloc[0].to_dict()
#             matched["Motherboard"] = best_mobo
#             total_cost += best_mobo["original_price"]
...
#             best_ram = compatible_ram.sort_values("original_price").iloc[0].to_dict()
#             matched["RAM"] = best_ram
#             total_cost += best_ram["original_price"]

#     # ✅ PSU
#     cheapest_psu = psu_data.sort_values("original_price").iloc[0].to_dict()
#     matched["PSU"] = cheapest_psu
#     total_cost += cheapest_psu["original_price"]

#     # ✅ Case
#     cheapest_case = case_data.sort_values("original_price").iloc[0].to_dict()
#     matched["Case"] = cheapest_case
#     total_cost += cheapest_case["original_price"]

#     # ✅ Spare Budget Upgrade Step
...
#     # ✅ Final
#     return matched, round(sum(comp["original_price"] for comp in matched.values() if comp), 2)
