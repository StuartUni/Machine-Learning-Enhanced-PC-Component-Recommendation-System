...
# Description: Loads and runs the collaborative filtering model (TFRS) to predict user preferences based on build ratings.
...
MODEL_PATH = os.path.join(BASE_DIR, "models", "tfrs_model")  # Ensure this path matches saved model
...
def recommend_builds(user_id: str, model=None, k: int = 10) -> List[str]:
    ...
    try:
        ...
        print("⚠️ Could not rebuild model index:", e)
